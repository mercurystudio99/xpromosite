...
const HeadwearMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch headwear categories using the standard pattern
  useEffect(() => {
    const fetchHeadwearCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/headwear');
        
        if (!response.ok) {
          throw new Error('Failed to fetch headwear categories');
        }
        ...
        // Custom order for headwear categories
        const desiredOrder = [
          'CAPS',
          'HATS',
          'BEANIES',
...
export default HeadwearMegamenu; 