...
const EcoProductsMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch eco-products categories using the standard pattern
  useEffect(() => {
    const fetchEcoProductsCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/eco-products');
        
        if (!response.ok) {
          throw new Error('Failed to fetch eco-products categories');
        }
        ...
        // Sort categories: those with items first, empty categories last
        ...
        console.error('Error fetching eco-products categories:', error);
...
export default EcoProductsMegamenu; 