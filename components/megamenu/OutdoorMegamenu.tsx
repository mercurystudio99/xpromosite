...
const OutdoorMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch outdoor categories using the standard pattern
  useEffect(() => {
    const fetchOutdoorCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/outdoor');
        
        if (!response.ok) {
          throw new Error('Failed to fetch outdoor categories');
        }
        ...
        // Custom order for outdoor categories
        const desiredOrder = [
          'UMBRELLAS',
          'SPORTS',
          'FITNESS',
        ...
        console.error('Error fetching outdoor categories:', error);
...
export default OutdoorMegamenu; 