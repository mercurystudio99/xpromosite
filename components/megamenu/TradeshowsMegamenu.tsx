...
const TradeshowsMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch tradeshows categories using the standard pattern
  useEffect(() => {
    const fetchTradeshowsCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/tradeshows');
        
        if (!response.ok) {
          throw new Error('Failed to fetch tradeshows categories');
        }
        ...
        // Custom order for tradeshows categories
        const desiredOrder = [
          'LANYARD',
          'GIVEAWAYS',
          'DISPLAYS',
        ...
        console.error('Error fetching tradeshows categories:', error);
...
export default TradeshowsMegamenu; 