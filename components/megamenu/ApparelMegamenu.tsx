...
const ApparelMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch apparel categories using the standard pattern
  useEffect(() => {
    const fetchApparelCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/apparel');
        
        if (!response.ok) {
          throw new Error('Failed to fetch apparel categories');
        }
        ...
        // Reorder categories to put desired items in first row
        const desiredOrder = ['T-Shirts', 'Hoodies & Sweatshirts', 'Polos', 'Jackets'];
        const reorderedCategories: Category[] = [];
        
        // Add categories in desired order first
        desiredOrder.forEach(desiredName => {
          const category = transformedCategories.find((cat: Category) => cat.title === desiredName);
          if (category) {
            reorderedCategories.push(category);
          }
        });
        
        // Add remaining categories that weren't in the desired order
        // Sort them by whether they have items (non-empty first, empty last)
        const remainingCategories = transformedCategories
          .filter((category: Category) => !desiredOrder.includes(category.title))
          .sort((a: Category, b: Category) => {
            // Categories with items come first, empty categories last
            if (a.items.length > 0 && b.items.length === 0) return -1;
            if (a.items.length === 0 && b.items.length > 0) return 1;
            return 0; // Keep original order for categories of same type
          });
...
export default ApparelMegamenu; 