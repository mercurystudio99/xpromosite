...
const TechMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch tech categories using the standard pattern
  useEffect(() => {
    const fetchTechCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/tech');
        ...
        // Custom order for tech categories
        const desiredOrder = [
          'PHONE',
          'SPEAKER',
          'CABLES',
