...
const BagsMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch bags categories using the standard pattern
  useEffect(() => {
    const fetchBagsCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/bags');
        
        if (!response.ok) {
          throw new Error('Failed to fetch bags categories');
        }
        ...
        // Custom sort: TOTE BAGS first, then others sorted by items availability
        const sortedCategories = transformedCategories.sort((a: Category, b: Category) => {
          // TOTE BAGS always comes first
          if (a.title.toUpperCase().includes('TOTE')) return -1;
          if (b.title.toUpperCase().includes('TOTE')) return 1;
          
          // For non-TOTE categories: those with items first, empty categories last
        ...
        console.error('Error fetching bags categories:', error);
