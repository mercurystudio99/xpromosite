...
const DrinkwareMegamenu: React.FC = () => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Fetch drinkware categories using the standard pattern
  useEffect(() => {
    const fetchDrinkwareCategories = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch('/api/category-tree/drinkware');
        
        if (!response.ok) {
          throw new Error('Failed to fetch drinkware categories');
        }
        ...
        // Custom order for drinkware categories
        const desiredOrder = [
          'WATER BOTTLES',
          'REUSABLE CUPS', 
          'TUMBLERS',
        ...
        console.error('Error fetching drinkware categories:', error);
...
export default DrinkwareMegamenu; 